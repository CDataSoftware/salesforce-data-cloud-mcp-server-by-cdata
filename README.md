...
CData's Model Context Protocol (MCP) Server for Salesforce Data Cloud

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Salesforce Data Cloud (beta)](https://www.cdata.com/download/download.aspx?sku=LRZM-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Salesforce Data Cloud supported by the [CData JDBC Driver for Salesforce Data Cloud](https://www.cdata.com/drivers/salesforcedc/jdbc).

CData JDBC Driver connects to Salesforce Data Cloud by exposing them as relational SQL models.

This server wraps that driver and makes Salesforce Data Cloud data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/salesforcedc/download/jdbc](https://www.cdata.com/drivers/salesforcedc/download/jdbc)
3. License the CData JDBC Driver:
    ...
    * Run the command `java -jar cdata.jdbc.salesforcedatacloud.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.salesforcedatacloud.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `salesforce-data-cloud.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.salesforcedatacloud.SalesforceDataCloudDriver)
